...
const LeaveReportPage = () => {
  ...
  const fetchReports = useCallback(async () => {
    try {
      ...
      const dailyUrl = query.toString()
        ? `${API_PATHS.LEAVE_REPORT_DAILY}?${query.toString()}`
        : API_PATHS.LEAVE_REPORT_DAILY;

      const [realtimeRes, dailyRes, pendingRes] = await Promise.all([
        axiosInstance.get(API_PATHS.LEAVE_REPORT_REALTIME),
        axiosInstance.get(dailyUrl),
        axiosInstance.get(API_PATHS.LEAVE_REPORT_PENDING),
      ]);
  ...
  const totalPendingDays = useMemo(() => {
    return pending.reduce((sum, item) => sum + (item.daysCount || 0), 0);
  }, [pending]);

  // Calculate total pending item days
  const totalPendingItems = useMemo(() => {
    return pending.reduce((sum, item) => sum + (item.daysPending || 0), 0);
  }, [pending]);
  ...
  const exportReport = async (type) => {
    try {
      ...
      const response = await axiosInstance.get(
        API_PATHS.LEAVE_REPORT_EXPORT(type, fromDate, toDate),
        { responseType: "blob" },
      );
      ...
      link.download = type === "csv" ? "leave-report.csv" : "leave-report.xlsx";
              ...
              {realtime.latestActivities.map((item) => (
                ...
                >
                  ...
                  <p className="text-xs text-slate-600">
                    {LEAVE_TYPES[item.leaveType]} ({item.daysCount} ngày)
                  </p>
                  <p className="text-xs text-slate-600">
        ...
        <article className="rounded-xl border border-slate-200 bg-white p-5 shadow-sm">
          <div className="mb-3 flex items-center justify-between">
            ...
            <p className="text-xs text-amber-600">
              {pending.length} yêu cầu | Tổng: {totalPendingDays} ngày
            </p>
          </div>
              ...
              {pending.map((item) => (
                ...
                >
                  ...
                  <p className="text-xs text-slate-700">
                    Loại: {LEAVE_TYPES[item.leaveType]}
                  </p>
                  <p className="text-xs text-slate-700">
                  ...
                  <p className="text-xs text-slate-600">Lý do: {item.reason}</p>
                  <p className="text-xs text-slate-500">
